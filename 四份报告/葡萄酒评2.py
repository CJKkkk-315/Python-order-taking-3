    ...
    for i in wine_list:
        if i[1]:
            try:
                ...
                d[i[1]] = [i[3]]
    res = [[i, round(sum(d[i]) / len(d[i]), 2)] for i in d.keys()]
    return res
...
def top_10_point(wine_list):
    ...
    @参数 wine_list：葡萄酒数据，列表类型
    """
    wine_list.sort(key=lambda x: x[3])
    wine_list = wine_list[::-1]
    res = [[i[0], i[1], i[3], i[4]] for i in wine_list[:10]]
    return res
...
def top_20_price(wine_list):
    ...
    nan的数据类型是float,不可以直接用字符串判定方法。
    """
    wine_list = [i for i in wine_list if not math.isnan(i[4])]
    wine_list.sort(key=lambda x: x[4])
    wine_list = wine_list[::-1]
    res = [[i[0], i[1], i[3], i[4]] for i in wine_list[:20]]
    return res
...
def amount_of_point(wine_list):
    ...
    @参数 wine_list：葡萄酒数据，列表类型
    """
    wine_list = [i[3] for i in wine_list if not math.isnan(i[3])]
    d = {}
    for i in wine_list:
        try:
            d[i] += 1
