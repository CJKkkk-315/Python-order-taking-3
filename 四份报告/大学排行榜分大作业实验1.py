...
def only_alumni(alumni, soft):
    ...
    """
    return set([i for i in list(alumni) if i not in list(soft)])
...
def only_one(alumni, soft):
    ...
    但不同时出现在两个榜单的学校名。
    """
    return alumni.difference(soft).union(soft.difference(alumni))
